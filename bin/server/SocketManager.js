...
let communityChat2 = createChat({name: 'chat 2'})

let connectedUsers = {};

let chats = [communityChat, communityChat2];
...
module.exports = function(socket){
  ...
  socket.on(USER_CONNECTED, function(user){
    ...
    connectedResponse.users = connectedUsers;
    connectedResponse.newUser = user;
    socket.user = user.name;
    sendMessageToChatFromUser = sendMessageToChat(user.name)
    sendTypingFromUser = sendTypingToChat(user.name)

    console.log(connectedUsers);
    io.emit(USER_CONNECTED, connectedResponse)
  ...
  socket.on('disconnect', function (){
    if(!!socket.user){
      ...
      connectedResponse.users = connectedUsers;
      connectedResponse.oldUser = socket.user;
      io.emit(USER_DISCONNECTED, connectedResponse)
    }
